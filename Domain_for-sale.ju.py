...
def process_list_for_sale_response(response_json):
    ...
    for j in range(len(response_json)):
        ...
        ##print(k)
        k['price'] = response_json[j]['listing']['priceDetails']['displayPrice']
        ##print(p)
...
# %%
# setup
property_id="2016858650" # selct a random  property in the area 
## TODO - insert search parameters
...
price=r['priceDetails']['displayPrice']
# the below puts all relevant property types into a single string. 
# eg. instead of a property listing as a both a 'house' and 'townhouse'
n=0
property_type_str=""
...
print(property_type_str, postcode, suburb, bedrooms, "Beds", bathrooms, "bath(s)",  carspaces, "car spaces(s)", area, "sqm", geolocation)
print(price)
# %%

property_list_for_sale_response = api_property_list_for_sale(auth, property_type, bedrooms, bathrooms, suburb, postcode)
property_list_for_sale_response[:1]
...
try:
    df_properties_for_sale_raw = df_properties_for_sale_raw.drop(['allPropertyTypes','buildingArea','labels','tags'], axis = 1)
    print("Done!")
except:
   print("Done!") 
